...
- `Scaling` -This has not yet been tested for the windows version. In both the `terrform/azure` and `terraform/aws` plans there is a `count` variable which allows you to scale out the web instances to demonstrate the concept of choreography vs orchestration in Habitat.
...
1. Clone this repo
2. `cd national-parks`
3. Export environment variables to forward ports on the `$env:HAB_DOCKER_OPTS ='-p 8000:8000 -p 8080:8080 -p 8085:8085 -p 9631:9631'`
4. `hab studio enter`
5. `build`
6. `. .\results\last_build.ps1`
7. Load `winhab/mongodb` package from the public depot:  
  `hab svc load winhab/mongodb`
8. Override the default configuration of mongodb:
   `hab config apply mongodb.default 50 mongo.toml`
9. Load the most recent build of national-parks: 
   `hab svc load $pkg_ident --bind database:mongodb.default`
10. Load `core/haproxy` from the public depot: `hab svc load core/haproxy --bind backend:national-parks.default` This is not currently working with Windows
11. Override the default configuration of HAProxy: `hab config apply haproxy.default $(date +%s) haproxy.toml` This is not currently working with Windows
12. Run `Get-SupervisorLog` to see the output of the supervisor
...
2. Bump the `$pkg_version` in `habitat/plan.ps1`
